#include <stdio.h>
// Desafio Super Trunfo - Países
// Tema 1 - Cadastro das cartas
// Objetivo: No nível novato você deve criar as cartas representando as cidades utilizando scanf para entrada de dados e printf para exibir as informações.
/*Carta 1:                      Carta 2:
Estado: A                       Estado: B
Código: A01                     Código: B02
Nome da Cidade: São Paulo       Nome da Cidade: Rio de Janeiro
População: 12325000             População: 6748000
Área: 1521.11 km²               Área: 1200.25 km²
PIB: 699.28 bilhões de reais    PIB: 300.50 bilhões de reais
Número de Pontos Turísticos: 50 Número de Pontos Turísticos: 30 */

int main() {

  // definição das variáveis

    char estado1,estado2;
    char codCarta1[10],codCarta2[10];
    char nomeCidade1[50],nomeCidade2[50],*vencedor1,*vencedor2;
    int populacao1,populacao2,numTuristico1,numTuristico2,opcao,opcao2,opcao3;
    int resultado1,resultado2;
    float area1,area2;
    float pib1,pib2;
    float DensidadePopulacional1,DensidadePopulacional2;
    float pibpercap1,pibpercap2;
    float superpoder1,superpoder2;
    
    //menu inicial 
    printf("*****************\n");
    printf("* Super Trunfo! *\n");
    printf("*****************\n");
   
    printf("\n");
    printf("Escolha uma opção\n");
    printf("1. Jogar\n");
    printf("2. Regras\n");
    printf("3. Sair\n");
    scanf("%d",&opcao);
    printf("\n");
//escolhas do menu
switch (opcao)
{
  case 1:
      // Área para entrada de dados das CARTA ALTERNADO, PARA USAR O FGETS PRIMEIRO SEM DAR ERRO(depois do swite não funciona só se usar while((getchar()) != '\n') para limpar o buffer)
  while ((getchar()) != '\n');
  
  //NOME CIDADE
    printf("Digite o nome cidade da CARTA 1: ");
    fgets(nomeCidade1, 50, stdin);
      printf("Digite o nome cidade da CARTA 2: ");
      fgets(nomeCidade2, 50, stdin);

  // ESTADO DA CARTA A,B,C,...
    printf("Digite o Estado da CARTA 1: ");
    scanf(" %c", &estado1);
      printf("Digite o Estado da CARTA 2: ");
      scanf(" %c", &estado2);

  // CODIGO DA CARTA
    printf("Digite o Código da CARTA 1: ");
    scanf("%s", &codCarta1);
      printf("Digite o Código da CARTA 2: ");
      scanf("%s", &codCarta2); 

  //POPULAÇÃO
    printf("Digite a população da CARTA 1: ");
    scanf("%d", &populacao1);
      printf("Digite a população da CARTA 2: ");
     scanf("%d", &populacao2);    

  //AREA DE TERRITORIO
    printf("Digite a Área da CARTA 1: ");
    scanf("%f", &area1);
      printf("Digite a Área da CARTA 2: ");
      scanf("%f", &area2);   

  //PIB
    printf("Digite o PIB da CARTA 1: ");
    scanf("%f", &pib1);
      printf("Digite o PIB da CARTA 2: ");
      scanf("%f", &pib2);

  //PONTOS TURISTICOS
    printf("Digite a quantidade de pontos turísticos da CARTA 1 : ");
    scanf("%d", &numTuristico1);
      printf("Digite a quantidade de pontos turísticos da CARTA 2 : ");
      scanf("%d", &numTuristico2);
    printf("\n");

    //tira o quebra linha da fgets
    nomeCidade1[strcspn(nomeCidade1, "\n")] = 0;
    nomeCidade2[strcspn(nomeCidade2, "\n")] = 0;

   //cálculos Densidade e pib per Capita 
   DensidadePopulacional1 = populacao1/area1;
   DensidadePopulacional2 = populacao2/area2;
   pibpercap1 = (float)pib1/populacao1;
   pibpercap2 = (float)pib2/populacao2;
   superpoder1 = (float)populacao1+area1+pib1+numTuristico1+pibpercap1+DensidadePopulacional1;
   superpoder2 = (float)populacao2+area2+pib2+numTuristico2+pibpercap2+DensidadePopulacional2;
      
  // Área para exibição dos dados da cidade 1

      printf("Carta 1\n");
      printf("Estado: %c\n",estado1);
      printf("Código da Carta: %s\n",codCarta1);
      printf("Nome da cidade: %s \n",nomeCidade1);
      printf("População: %d\n", populacao1);
      printf("Área: %.2f km²\n", area1);
      printf("Pib: %.2f bilhões de reais\n",pib1);    
      printf("Nº de pontos Turísticos: %d\n",numTuristico1);
      printf("Densidade Populacional: %.2f \n",DensidadePopulacional1);
      printf("PIB per Capita: %f\n",pibpercap1);
      printf("Super Poder da Carta 1 : %f \n", superpoder1);
      printf("\n");

  // Área para exibição dos dados da cidade 2

      printf("Carta 2\n");
      printf("Estado: %c\n",estado2);
      printf("Código da Carta: %s \n",codCarta2);
      printf("Nome da cidade: %s \n",nomeCidade2);
      printf("População: %d \n", populacao2);
      printf("Área: %.2f km² \n", area2);
      printf("Pib: %.2f bilhões de reais \n",pib2);
      printf("Nº de pontos Turísticos: %d \n",numTuristico2);
      printf("Densidade Populacional: %.2f \n",DensidadePopulacional2);
      printf("PIB per Capita: %f \n",pibpercap2);
      printf("Super Poder da CARTA 2 : %f  \n",  superpoder2);
      printf("\n");

/* -->**comparação simples** **DESATIVADO**
      printf("Escolha algum atributo para comparar:\n");
      printf("1. População.\n");
      printf("2. Área.\n");
      printf("3. Pib.\n");
      printf("4. Pontos Turisticos.\n");
      printf("5. Densidade Populacional.\n");
      printf("6. Pib per capita. \n");
      printf("7. Super Poder. \n");
      printf("\n");
      scanf("%d",&opcao2);

      switch (opcao2)
      {
      case 1:
      
          if (populacao1 == populacao2)
          {
           printf("Empate\n");
           printf("\n");

          } else if (populacao1 > populacao2)
          {
            printf("Cidade 1 %s   - Cidade 2 %s \n",nomeCidade1,nomeCidade2);
            printf("População: %d - %d \n",populacao1,populacao2);
            printf("Carta 1 Venceu! \n");
            printf("\n");

          }else {
            printf("Cidade 1 %s   - Cidade 2 %s \n",nomeCidade1,nomeCidade2);
            printf("População: %d - %d \n",populacao1,populacao2);
            printf("Carta 2 Venceu! \n");
            printf("\n");

          }
          
        break;

        case 2:

        if (area1 == area2)
          {
           printf("Empate\n");
           printf("\n");
          } else if (area1 > area2)
          {
            printf("Cidade 1 %s   - Cidade 2 %s \n",nomeCidade1,nomeCidade2);
            printf("Área: %.2f km² - %.2f km² \n",area1,area2);
            printf("Carta 1 Venceu! \n");
            printf("\n");

          }else {
            printf("Cidade 1 %s   - Cidade 2 %s \n",nomeCidade1,nomeCidade2);
            printf("Área: %.2f km² -%.2f km² \n",area1,area2);
            printf("Carta 2 Venceu! \n");
            printf("\n");

          }
          
          break;
          case 3:
            if (pib1 == pib2)
          {
           printf("Empate\n");
           printf("\n");

          } else if (pib1 > pib2)
          {
            printf("Cidade 1 %s   - Cidade 2 %s\n",nomeCidade1,nomeCidade2);
            printf("Pib: %f - %f\n",pib1,pib2);
            printf("Carta 1 Venceu! \n");
            printf("\n");

          }else {
            printf("Cidade 1 %s   - Cidade 2 %s\n",nomeCidade1,nomeCidade2);
            printf("Pib: %f - %f\n",pib1,pib2);
            printf("Carta 2 Venceu! \n");
            printf("\n");

          }
          
          break;

          case 5:

          if (DensidadePopulacional1 == DensidadePopulacional2)
          {
           printf("Empate\n");
           printf("\n");

          } else if (DensidadePopulacional1 < DensidadePopulacional2)
          {
            printf("Cidade 1 %s   - Cidade 2 %s\n",nomeCidade1,nomeCidade2);
            printf("Densidade Populacional: %f - %f\n",DensidadePopulacional1,DensidadePopulacional2);
            printf("Carta 1 Venceu! \n");
            printf("\n");

          }else {
            printf("Cidade 1 %s   - Cidade 2 %s \n",nomeCidade1,nomeCidade2);
            printf("Densidade Populacional: %f - %f \n",DensidadePopulacional1,DensidadePopulacional2);
            printf("Carta 2 Venceu! \n");
            printf("\n");

          }       
          break;

          case 6:
          if (pibpercap1 == pibpercap2)
          {
           printf("Empate\n");
           printf("\n");

          } else if (pibpercap1 > pibpercap2)
          {
            printf("Cidade 1 %s   - Cidade 2 %s \n",nomeCidade1,nomeCidade2);
            printf("Pib per capita: %f - %f \n",pibpercap1,pibpercap2);
            printf("Carta 1 Venceu! \n");
            printf("\n");

          }else {
            printf("Cidade 1 %s   - Cidade 2 %s \n",nomeCidade1,nomeCidade2);
            printf("Pib per capita: %f - %f \n",pibpercap1,pibpercap2);
            printf("Carta 2 Venceu! \n");
            printf("\n");

          }  
          break;
          case 7:
          if (superpoder1 == superpoder2)
          {
           printf("Empate\n");
           printf("\n");

          } else if (superpoder1 > superpoder2)
          {
            printf("Cidade 1 %s   - Cidade 2 %s \n",nomeCidade1,nomeCidade2);
            printf("Super Poder: %f - %f \n",superpoder1,superpoder2);
            printf("Carta 1 Venceu! \n");
            printf("\n");

          }else {
            printf("Cidade 1 %s   - Cidade 2 %s \n",nomeCidade1,nomeCidade2);
            printf("Super Poder: %d - %d \n",superpoder1,superpoder2);
            printf("Carta 2 Venceu! \n");
            printf("\n");

          }  

          break;
      
      default:

          printf("Opção invalida! \n");
          printf("\n");

        break;
      }<--- *** FIM DA COMPARAÇÃO SIMPLES ** DESATIVADO** */

      //comparação dupla

      printf("Agora vamos escolher dois atributo para comparar!\n");
      printf("Escolha o primeiro atributo para comparar:\n");

      // primeiro atributo
      printf("1. População.\n");
      printf("2. Área.\n");
      printf("3. Pib.\n");
      printf("4. Pontos Turisticos.\n");
      printf("5. Densidade Populacional.\n");
      printf("6. Pib per capita. \n");
      printf("7. Super Poder. \n");
      printf("\n");
      scanf("%d",&opcao2);

      //segundo atributo
      printf("Escolha o Segundo atributo para comparar:\n");
      printf("1. População.\n");
      printf("2. Área.\n");
      printf("3. Pib.\n");
      printf("4. Pontos Turisticos.\n");
      printf("5. Densidade Populacional.\n");
      printf("6. Pib per capita. \n");
      printf("7. Super Poder. \n");
      printf("\n");
      scanf("%d",&opcao3);

      //escolha do primeiro atributo
    switch (opcao2)
      {
      case 1:
          printf("Cidade 1: %s  -  Cidade 2: %s \n",nomeCidade1,nomeCidade2);
          printf("População: %d  -  %d \n",populacao1,populacao2);
          
          resultado1 = populacao1 > populacao2 ? 1 : 0;

        break;
      case 2:
          printf("Cidade 1: %s  -  Cidade 2: %s \n",nomeCidade1,nomeCidade2);
          printf("Área: %f  -  %f \n",area1,area2);
          
          resultado1 = area1 > area2 ? 1 : 0;
        break;
      case 3:
         printf("Cidade 1: %s  -  Cidade 2: %s \n",nomeCidade1,nomeCidade2);
         printf("PIB: %f  -  %f \n",pib1,pib2);
          
         resultado1 = pib1 > pib2 ? 1 : 0;

       break;
      case 4:
        printf("Cidade 1: %s  -  Cidade 2: %s \n",nomeCidade1,nomeCidade2);
        printf("Nº Turisticos: %d  -  %d \n",numTuristico1,numTuristico2);
          
        resultado1 = numTuristico1 > numTuristico2 ? 1 : 0;

       break;
      case 5:
        printf("Cidade 1: %s  -  Cidade 2: %s \n",nomeCidade1,nomeCidade2);
        printf("Densidade populacioanal: %f  -  %f \n",DensidadePopulacional1,DensidadePopulacional2);
          
          resultado1 = DensidadePopulacional1 < DensidadePopulacional2? 1 : 0;

       break;
      case 6:
        printf("Cidade 1: %s  -  Cidade 2: %s \n",nomeCidade1,nomeCidade2);
        printf("PIB per Capita: %f  -  %f \n",pibpercap1,pibpercap2);
          
          resultado1 = pibpercap1 > pibpercap2 ? 1 : 0;
       
      break;
      case 7:
        printf("Cidade 1: %s  -  Cidade 2: %s \n",nomeCidade1,nomeCidade2);
        printf("Super poder: %f  -  %f \n",superpoder1,superpoder2);
          
          resultado1 = superpoder1 > superpoder2 ? 1 : 0;
       break;

      default:
       printf("Opção invalida!");
        break;
      }

    switch (opcao3)
    {
       case 1:
       if(opcao2 == opcao3){
        printf("Atributo escolhido igual, favor escolha atributos diferentes!\n");


       }else{
          printf("Cidade 1: %s  -  Cidade 2: %s \n",nomeCidade1,nomeCidade2);
          printf("População: %d  -  %d \n",populacao1,populacao2);
          
          resultado2 = populacao1 > populacao2 ? 1 : 0;
       }
        break;
      case 2:
      if(opcao2 == opcao3){
        printf("Atributo escolhido igual, favor escolha atributos diferentes!\n");

       }else{
          printf("Cidade 1: %s  -  Cidade 2: %s \n",nomeCidade1,nomeCidade2);
          printf("Área: %f  -  %f \n",area1,area2);
          
          resultado2 = area1 > area2 ? 1 : 0;
       }
        break;
      case 3:
      if(opcao2 == opcao3){
        printf("Atributo escolhido igual, favor escolha atributos diferentes!\n");

       }else{
         printf("Cidade 1: %s  -  Cidade 2: %s \n",nomeCidade1,nomeCidade2);
         printf("PIB: %f  -  %f \n",pib1,pib2);
          
         resultado2 = pib1 > pib2 ? 1 : 0;
       }
       break;
      case 4:
      if(opcao2 == opcao3){
        printf("Atributo escolhido igual, favor escolha atributos diferentes!\n");

       }else{
        printf("Cidade 1: %s  -  Cidade 2: %s\n",nomeCidade1,nomeCidade2);
        printf("Nº Turistico: %d  -  %d \n",numTuristico1,numTuristico2);
          
        resultado2 = numTuristico1 > numTuristico2 ? 1 : 0;
       }
       break;
      case 5:
        if(opcao2 == opcao3){
        printf("Atributo escolhido igual, favor escolha atributos diferentes!\n");

       }else{
        printf("Cidade 1: %s  -  Cidade 2: %s \n",nomeCidade1,nomeCidade2);
        printf("Densidade Populacional: %f  -  %f \n",DensidadePopulacional1,DensidadePopulacional2);
          
          resultado2 = DensidadePopulacional1 < DensidadePopulacional2? 1 : 0;
       }
       break;
      case 6:
        if(opcao2 == opcao3){
        printf("Atributo escolhido igual, favor escolha atributos diferentes!\n");

       }else{
        printf("Cidade 1: %s  -  Cidade 2: %s \n",nomeCidade1,nomeCidade2);
        printf("PIB per Capita: %f  -  %f \n",pibpercap1,pibpercap2);
          
          resultado2= pibpercap1 > pibpercap2 ? 1 : 0;
       }
      break;
      case 7:
        if(opcao2 == opcao3){
        printf("Atributo escolhido igual, favor escolha atributos diferentes!\n");

       }else{
        printf("Cidade 1: %s  -  Cidade 2: %s \n",nomeCidade1,nomeCidade2);
        printf("Super poder: %f  -  %f \n",superpoder1,superpoder2);
          
          resultado2 = superpoder1 > superpoder2 ? 1 : 0;
        }

       break;
        
       default:
       printf("Opção invalida!");
        break;
    }        

  break;
  case 2:

     printf("O jogo se trata de um duelo de cartas\n");
     printf("O sistema solicita as entradas alternadas da Carta 1 e depois da Carta 2.\n");
     printf("São pedido os dados como Nome da cidade, População,PIB, área m² \n\n");
  
  break;
  case 3:
     
     printf("Jogo finalizado.\n");
     printf("\n");
  break;
    
  default:
  printf("Opção invalida!.");
  break;
}
printf("\n");

vencedor1 = (resultado1 == 1  ? "Carta 1 Venceu" : "Carta 2 Venceu");
printf("vencedor1: %s\n", vencedor1);
vencedor2 = (resultado2 == 0  ? "Carta 2 Venceu" : "Carta 1 Venceu");
printf("vencedor2: %s\n", vencedor2);
printf("\n");

if((vencedor1 == "Carta 1 Venceu" && vencedor2 == "Carta 1 Venceu")){
  printf("Parabéns, Carta 1 Venceu!\n");
}else if ((vencedor1 == "Carta 2 Venceu" && vencedor2 == "Carta 2 Venceu")){
  printf("Parabéns, Carta 2 Venceu!\n");
}else {
  printf("Que pena deu Empate!\n");
}


//espaço final
 printf("\n");

return 0;

} 
